...
## Headline (7B): the debugger reward beats compile-only RLVR — zero-leak

The full result: **DoRA SFT on Qwen2.5-Coder-7B (6000 steps, 208,749 shaders) → debugger-in-the-loop
GRPO RL**, with the C++ OmniTrace `omni_reward` (compile + CPU-run + **real GPU render**: finite +
luminance-variance) as the reward. Evaluated on **40 held-out natural-language prompts verified absent
from the SFT corpus** (we caught and removed 32/40 prompt leakage first — the leaked numbers are a
memorization ceiling, not reported). compile@1 / run@1 / render@1 / mean-reward:

| Model | compile@1 | run@1 | render@1 | meanR |
|---|---|---|---|---|
| base 7B | 0.0 | 0.0 | 0.0 | 0.044 |
| + DoRA SFT | 0.90 | 0.875 | 0.775 | 3.794 |
| + RL compile-only (150 steps) | 0.975 | 0.975 | 0.875 | 3.835 |
| **+ RL debugger (160 steps)** | **1.0** | **1.0** | **0.95** | **4.056** |
| **deliver@400** (debugger-RL to convergence) | **1.0** | **1.0** | **1.0** | **5.308** |

**The contribution — debugger reward vs compile-only RLVR at matched compute (160 vs 150 steps):**
the debugger wins every metric and is the only row to reach **perfect compile@1 + run@1** (zero NaN
shaders); gains concentrate in render@1 (**+0.075**) and mean reward (**+0.221**) — the run/render
quality a compile-only reward is *blind* to. Magnitude is modest because strong SFT already nears the
compile ceiling; the debugger's edge is exactly the dimension it uniquely sees.

**The deliverable (`deliver@400`)** is prompt-faithful (6/6 distinct shaders for 6 distinct prompts —
not mode-collapsed; verified by rendering), perfect on its trained NL distribution. **Honest OOD:** on
arbitrary free-form prompts it drops to **compile@1 0.625 / render@1 0.575 (compile@k 0.90)** — strong
on its prompt style, ~60% on anything; renders tend monochrome. Rendered samples in
`trained_models/samples400b/` (spiral, kaleidoscope, etc. — real, distinct, prompt-relevant).

## Headline (3B precedent): the debugger-in-the-loop RL works
